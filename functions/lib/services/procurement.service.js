...
const requestFlow_model_1 = __importDefault(require("../models/requestFlow.model")); // Import RequestFlow model
...
class ProcurementService {
    async createProcurement(data) {
        const procurementFlow = await requestFlow_model_1.default.findOne({ workflowItem: 'Procurement' });
        if (!procurementFlow) {
        ...
        if (!data.category || data.category !== 'Procurement') {
            throw new customError_1.default('Category must be "procurement"', 400);
        }
        ...
        if (!budget) {
            throw new customError_1.default('No budget found with a category titled "procurement"', 404);
        }
        ...
        const procurement = new procurement_model_1.default(procurementData);
        return await procurement.save();
    }
    async processRequest(procurementId, data) {
        ...
        try {
            ...
            const procurement = await procurement_model_1.default.findById(procurementId).session(session);
            if (!procurement) {
                console.error("Procurement not found for ID:", procurementId);
                throw new customError_1.default('Procurement not found', 404);
            }
            ...
            if (!procurement.procurementTitle || !procurement.category || !procurement.purposeOfRequest) {
                ...
                throw new customError_1.default('Procurement title, category, and purpose of request are required', 400);
            }
            ...
            if (!procurement.budgetId) {
                ...
                throw new customError_1.default('Budget ID is required', 400);
            }
            // Step 4: Find the budget and validate
            const budget = await budget_model_1.default.findById(procurement.budgetId).session(session);
            if (!budget) {
            ...
            if (budget.status !== 'approved') {
                ...
                throw new customError_1.default('Budget has not been approved yet', 400);
            }
            ...
            if (!budgetCategory) {
                ...
                throw new customError_1.default('Procurement category not found in budget', 404);
            }
            ...
            if (!data.items || data.items.length !== procurement.items.length) {
                ...
                throw new customError_1.default('Items array must match procurement items', 400);
            }
            const budgetItemIds = data.items.map((item) => item.budgetItem);
            const budgetItems = budgetCategory.budgetItems.filter((bi) => bi._id && budgetItemIds.includes(bi._id.toString()));
            for (const [index, item] of data.items.entries()) {
                if (!item.budgetItem) {
                    console.error(`Budget item ID is required for item at index ${index}:`, item);
                    throw new customError_1.default(`Budget item ID is required for item at index ${index}`, 400);
                }
                ...
                if (!budgetItem) {
                    ...
                    throw new customError_1.default(`Budget item with ID ${item.budgetItem} not found in Procurement category`, 404);
                }
            }
            ...
            const updatedItems = await Promise.all(procurement.items.map(async (item, index) => {
                ...
                if (inputItem.vendorInventoryId) {
                    const inventory = await vendorInventory_model_1.default.findById(inputItem.vendorInventoryId).session(session);
                    if (!inventory) {
                        console.error(`Vendor inventory not found for item ${itemName} at index ${index}:`, inputItem.vendorInventoryId);
                        throw new customError_1.default(`Vendor inventory not found for item ${itemName}`, 404);
                    }
                    if (inventory.status !== 'available') {
                        console.error(`Vendor inventory not available for item ${itemName} at index ${index}:`, inventory.status);
                        throw new customError_1.default(`Vendor inventory for item ${itemName} is not available`, 400);
                    }
                    if (inventory.quantity < (inputItem.quantity ?? 0)) {
                        ...
                        throw new customError_1.default(`Insufficient quantity in vendor inventory for item ${itemName}`, 400);
                    }
                    ...
                    if (unitPrice == null || unitPrice <= 0) {
                        ...
                        throw new customError_1.default(`Unit price is required and must be positive for item ${itemName} without vendorInventoryId`, 400);
                    }
                ...
                if (!budgetItem) {
                    throw new customError_1.default(`Budget item ${inputItem.budgetItem} not found for item ${itemName}`, 404);
                }
                // Validate budgetItem amount for this item's cost
                if (budgetItem.amount < amount) {
                    console.error(`Budget item amount (${budgetItem.amount}) is less than item cost (${amount}) for item ${itemName} at index ${index}`);
                    throw new customError_1.default(`Budget item amount (${budgetItem.amount}) is insufficient for item ${itemName} cost (${amount})`, 400);
                }
                return {
                    ...item.toObject(),
                    itemName,
            ...
            const totalCost = updatedItems.reduce((sum, item) => sum + (item.amount || 0), 0);
            console.log("Calculated total cost:", totalCost);
            // Step 9: Validate budgetCategory amount
            if (budgetCategory.amount < totalCost) {
                console.error(`Budget category amount (${budgetCategory.amount}) is insufficient for total cost (${totalCost})`);
                throw new customError_1.default('Procurement category amount is insufficient for the total cost', 400);
            }
            // Step 10: Update the procurement document
            const updatedProcurement = await procurement_model_1.default.findByIdAndUpdate(procurementId, {
                items: updatedItems,
                totalCost,
                status: 'processed',
    ...
    async receiveItem(procurementId, itemId, userId) {
        ...
        try {
            ...
            if (!procurement.budgetId || !item.budgetItem) {
                ...
                throw new customError_1.default('Budget ID and budgetItem ID are required', 400);
            }
            ...
            if (!budgetItem) {
                ...
                throw new customError_1.default('Budget item not found in Procurement category', 404);
            }
            ...
            if (budgetItem.amount < item.actualAmount) {
                console.error(`Budget item amount (${budgetItem.amount}) is insufficient for item ${item.itemName} cost (${item.actualAmount})`);
                throw new customError_1.default(`Insufficient budget for item ${item.itemName}. Available: ${budgetItem.amount}, Required: ${item.actualAmount}`, 400);
            }
            ...
            procurement.lastUpdatedBy = new mongoose_1.default.Types.ObjectId(userId);
    ...
    async rejectItem(procurementId, itemId, userId) {
    ...
    async addItemToInventory(data) {
        ...
        if (!itemName || !quantity || quantity <= 0 || !procurementId || !procurementItemId || !addedBy) {
            throw new customError_1.default('itemName, quantity, procurementId, procurementItemId, and addedBy are required', 400);
        }
        ...
        try {
            ...
            const procurementItem = procurement.items.find((item) => item._id.toString() === procurementItemId);
            if (!procurementItem) {
            ...
            if (procurementItem.status !== 'accepted') {
                ...
                throw new customError_1.default(`Item ${itemName} must be accepted before adding to inventory`, 400);
            }
            if (procurementItem.addedToInventory) {
                console.warn(`Item ${itemName} is already added to inventory`);
                throw new customError_1.default(`Item ${itemName} is already added to inventory`, 400);
            }
            // Update or create inventory item
            let inventoryItem = await companyInventory_model_1.default.findOne({ itemName }).session(session);
            if (inventoryItem) {
                ...
                inventoryItem = new companyInventory_model_1.default({
                    itemName,
                    category: procurement.category || 'General',
                    quantity,
                    unitPrice: unitPrice || procurementItem.unitPrice || 0,
                    status: quantity > 0 ? 'in-stock' : 'out-of-stock',
                    createdBy: new mongoose_1.default.Types.ObjectId(addedBy),
                });
                await inventoryItem.save({ session });
            ...
            procurement.lastUpdatedBy = new mongoose_1.default.Types.ObjectId(addedBy);
            await procurement.save({ session });
            ...
            const updatedProcurement = await procurement_model_1.default.findById(procurementId)
            ...
            if (!updatedProcurement) {
                ...
                throw new customError_1.default('Updated procurement not found', 404);
            }
            return {
    ...
    async updateInventory(data) {
        ...
        try {
            ...
            if (procurementItem.status !== 'accepted') {
                console.error(`Item ${itemName} is not accepted, status: ${procurementItem.status}`);
                throw new customError_1.default(`Item ${itemName} must be accepted before updating inventory`, 400);
            }
            // Update or create inventory item
            let inventoryItem = await companyInventory_model_1.default.findOne({ itemName }).session(session);
            if (inventoryItem) {
    ...
    async getProcurements() {
        return await procurement_model_1.default.find()
            .populate({
            path: 'user',
    ...
    async getProcurementById(id) {
        const procurement = await procurement_model_1.default.findById(id)
            .populate({
            path: 'user',
            select: 'fullName email firstName lastName',
            strictPopulate: false,
    ...
    async updateProcurement(id, data) {
        ...
        if (data.items && Array.isArray(data.items)) {
            const updatedItems = await Promise.all(procurement.items.map(async (existingItem) => {
                // Match by _id if provided
                const updateItem = data.items.find((item) => item._id === existingItem._id?.toString()) || {};
                // Validate vendorInventoryId if provided
                if (updateItem.vendorInventoryId) {
                    const inventory = await vendorInventory_model_1.default.findById(updateItem.vendorInventoryId);
                    if (!inventory) {
                        ...
                        throw new customError_1.default(`Vendor inventory not found for item ${updateItem.itemName || existingItem.itemName} with vendorInventoryId ${updateItem.vendorInventoryId}`, 400);
                    }
                }
        ...
        const updatedProcurement = await procurement_model_1.default.findByIdAndUpdate(id, { $set: { ...data, items: data.items } }, // Explicitly set items to ensure subdocument update
    ...
    async deleteProcurement(id) {
        const procurement = await procurement_model_1.default.findByIdAndDelete(id);
        if (!procurement) {
            throw new customError_1.default('Procurement not found', 404);
        }
